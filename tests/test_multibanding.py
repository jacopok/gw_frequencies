...
from gw_frequencies.multibanding import (
    SEGLEN_20_HZ,
    high_frequency_grid,
    low_frequency_grid,
    mixed_frequency_grid,
    seglen_from_freq,
)
...
def test_reduced_frequency_array_simplifies_to_high_or_low_frequency_grids():
    ...
    assert np.allclose(
        mixed_frequency_grid(30.0, 40.0, 200.0), low_frequency_grid(30.0, 40.0)
    )
    assert np.allclose(
        mixed_frequency_grid(30.0, 40.0, 10.0), high_frequency_grid(30.0, 40.0)
    )
...
def test_reduced_frequency_array_histogram():
    arr = mixed_frequency_grid(30.0, 100.0, 40.0)
